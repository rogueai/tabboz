    ...
    pub fn to_color_chooser_widget(self: ColorChooser) ?ColorChooserWidget {
        if (self.as_widget().isa(ColorChooserWidget)) {
            return ColorChooserWidget{
                .ptr = @as(*c.GtkColorChooserWidget, @ptrCast(self.ptr)),
            };
        } else return null;
    }
...
pub const ColorButton = struct {
    ...
    pub fn new() Self {
        return Self{
            .ptr = @as(*c.GtkColorButton, @ptrCast(c.gtk_color_button_new())),
        };
    }

    pub fn new_sith_rgba(rgba: c.GdkRGBA) Self {
        return Self{
            .ptr = @as(*c.GtkColorButton, @ptrCast(c.gtk_color_button_new_with_rgba(rgba))),
        };
    }
