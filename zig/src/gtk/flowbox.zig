...
const FlowBox = struct {
    ...
    pub fn new() Self {
        return Self{ .ptr = @as(*c.GtkFlowBox, @ptrCast(c.gtk_flow_box_new())) };
    }
    ...
    pub fn set_selection_mode(self: Self, mode: SelectionMode) void {
        c.gtk_flow_box_swet_selection_mode(self.ptr, @intFromEnum(mode));
    }

    pub fn get_selection_mode(self: Self) SelectionMode {
        return @as(SelectionMode, @enumFromInt(c.gtk_flow_box_get_selection_mode(self.ptr)));
    }
...
const FlowBoxChild = struct {
    ...
    pub fn new() Self {
        return Self{
            .ptr = @as(*c.GtkFlowBoxChild, @ptrCast(c.gtk_flow_box_child_new())),
        };
    }
