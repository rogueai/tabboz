...
pub const Image = struct {
    ...
    pub fn get_storage_type(self: Self) Type {
        return @as(Type, @enumFromInt(c.gtk_image_get_storage_type(self.ptr)));
    }
    ...
    pub fn new_from_file(file: [:0]const u8) Self {
        return Self{
            .ptr = @as(*c.GtkImage, @ptrCast(c.gtk_image_new_from_file(file.ptr))),
        };
    }
    ...
    pub fn new_from_pixbuf(pixbuf: ?*c.GdkPixbuf) Self {
        return Self{
            .ptr = @as(*c.GtkImage, @ptrCast(c.gtk_image_new_from_pixbuf(if (pixbuf) |p| p else null))),
        };
    }
