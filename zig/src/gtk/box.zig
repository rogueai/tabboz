...
pub const Box = struct {
    ...
    pub fn new(orientation: Orientation, spacing: c_int) Self {
        return Self{
            .ptr = @as(*c.GtkBox, @ptrCast(c.gtk_box_new(@intFromEnum(orientation), spacing))),
        };
    }
    ...
    pub fn set_child_packing(
        ...
        c.gtk_box_set_child_packing(
            ...
            if (fill) 1 else 0,
            padding,
            @intFromEnum(pack_type),
        );
    }

    /// Gets the value set by gtk_box_set_baseline_position().
    pub fn get_baseline_position(self: Self) BaselinePosition {
        return @as(BaselinePosition, @enumFromInt(c.gtk_box_get_baseline_position(self.ptr)));
    }
    ...
    pub fn set_baseline_position(self: Self, pos: BaselinePosition) void {
        c.gtk_box_set_baseline_position(self.ptr, @intFromEnum(pos));
    }
